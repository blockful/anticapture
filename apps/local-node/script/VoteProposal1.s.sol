// apps/local-node/script/VoteProposal1.s.sol
...
/**
 * @title VoteProposal1
 * @dev Script to vote on the first governance proposal
 ...
 */
contract VoteProposal1 is BaseVoteProposal {
    ...
    /**
     * @dev Returns the voting configuration for this proposal
     */
    function getVoterConfigs() internal pure override returns (VoterConfig[] memory) {
        VoterConfig[] memory voters = new VoterConfig[](3);
        voters[0] = VoterConfig("Alice", Constants.ALICE_PRIVATE_KEY, Constants.FOR);
        voters[1] = VoterConfig("Bob", Constants.BOB_PRIVATE_KEY, Constants.FOR);
        voters[2] = VoterConfig("Charlie", Constants.CHARLIE_PRIVATE_KEY, Constants.FOR);
        return voters;
    }
