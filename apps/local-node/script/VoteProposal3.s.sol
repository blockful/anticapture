// apps/local-node/script/VoteProposal3.s.sol
...
/**
 * @title VoteProposal3
 * @dev Script to vote on the third governance proposal with different voting patterns
 ...
 */
contract VoteProposal3 is BaseVoteProposal {
    ...
    /**
     * @dev Returns the voting configuration for this proposal - different pattern with Alice AGAINST
     */
    function getVoterConfigs() internal pure override returns (VoterConfig[] memory) {
        ...
        voters[1] = VoterConfig("Bob", Constants.BOB_PRIVATE_KEY, Constants.FOR);          // Bob votes FOR
        voters[2] = VoterConfig("Charlie", Constants.CHARLIE_PRIVATE_KEY, Constants.FOR);  // Charlie votes FOR
        return voters;
    }
