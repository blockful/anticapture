  ...
  it("should correctly calculate mean with decimal precision", () => {
    const daoResponses = new Map<string, DelegationPercentageResponse>([
      [
        "ENS",
        {
          items: [{ date: "1600041600", high: "12.35" }],
          totalCount: 1,
          pageInfo: {
            hasNextPage: false,
            hasPreviousPage: false,
      ...
      [
        "UNI",
        {
          items: [{ date: "1600041600", high: "23.46" }],
          totalCount: 1,
          pageInfo: {
            hasNextPage: false,
            hasPreviousPage: false,
    ...
    expect(result[0].high).toBe("17.91");
  });
...
describe("buildPaginatedResponse", () => {
  ...
  it("should combine limit and hasNextPage from DAOs", () => {
    ...
    const result = buildPaginatedResponse(
      items,
      { orderDirection: "desc", limit: 2 },
      true,
    );

    expect(result.items).toHaveLength(2);
    expect(result.items[0].date).toBe("4");
  ...
  it("should calculate hasPreviousPage correctly", () => {
    ...
    const result1 = buildPaginatedResponse(
      items,
      { startDate: "1", after: undefined },
      false,
    );
    expect(result1.pageInfo.hasPreviousPage).toBe(false);

    const result2 = buildPaginatedResponse(
      items,
      { startDate: "1", after: "1" },
      false,
    );
    ...
    const result3 = buildPaginatedResponse(
      items,
      { startDate: "1", after: "2" },
      false,
    );
