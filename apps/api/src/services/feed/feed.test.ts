...
describe("FeedService", () => {
  ...
  describe("getFeedEvents", () => {
    ...
    it("should assign HIGH relevance when value >= HIGH threshold", async () => {
      const highThreshold =
        EventRelevanceThresholds[DaoIdEnum.ENS][FeedEventType.VOTE][
          FeedRelevance.HIGH
        ];
      mockRepo.getFeedEvents.mockResolvedValue({
    ...
    it("should assign correct relevance for each event in a mixed list", async () => {
      const ensThresholds =
        EventRelevanceThresholds[DaoIdEnum.ENS][FeedEventType.TRANSFER];
      mockRepo.getFeedEvents.mockResolvedValue({
        items: [
          createMockEvent({
            type: "TRANSFER",
            value: ensThresholds[FeedRelevance.HIGH],
            logIndex: 0,
          }),
          createMockEvent({
            type: "TRANSFER",
            value: ensThresholds[FeedRelevance.MEDIUM],
            logIndex: 1,
          }),
          createMockEvent({
            type: "TRANSFER",
    ...
    it("should pass correct value thresholds to repository for MEDIUM relevance", async () => {
      const ensThresholds = EventRelevanceThresholds[DaoIdEnum.ENS];

      await service.getFeedEvents(
        createRequest({ relevance: FeedRelevance.MEDIUM }),
      );

      expect(mockRepo.getFeedEvents).toHaveBeenCalledWith(
        expect.anything(),
        {
    ...
    it("should use NOUNS thresholds for NOUNS dao", async () => {
      ...
      const nounsThresholds = EventRelevanceThresholds[DaoIdEnum.NOUNS];
