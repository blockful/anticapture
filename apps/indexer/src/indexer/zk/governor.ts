...
export function GovernorIndexer(blockTime: number) {
  ...
  ponder.on(`ZKGovernor:VoteCast`, async ({ event, context }) => {
  ...
  ponder.on(`ZKGovernor:ProposalCreated`, async ({ event, context }) => {
  ...
  ponder.on(`ZKGovernor:ProposalCanceled`, async ({ event, context }) => {
  ...
  ponder.on(`ZKGovernor:ProposalExecuted`, async ({ event, context }) => {
    await updateProposalStatus(
      context,
      event.args.proposalId.toString(),
  ...
  ponder.on(`ZKGovernor:ProposalQueued`, async ({ event, context }) => {
    await updateProposalStatus(
      context,
      event.args.proposalId.toString(),
