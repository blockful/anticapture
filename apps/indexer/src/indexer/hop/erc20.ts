...
export function HOPTokenIndexer(address: Address, decimals: number) {
  const daoId = DaoIdEnum.HOP;

  ponder.on(`HOPToken:setup`, async ({ context }) => {
  ...
  ponder.on(`HOPToken:DelegateChanged`, async ({ event, context }) => {
  ...
  ponder.on(`HOPToken:DelegateVotesChanged`, async ({ event, context }) => {
    const { logIndex, address } = event.log;
    const { hash, from, to } = event.transaction;
