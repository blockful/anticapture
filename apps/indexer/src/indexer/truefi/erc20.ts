...
export function TrueFiTokenIndexer(
  address: Address,
  decimals: number,
  daoId: DaoIdEnum = DaoIdEnum.TRUEFI,
) {
  ponder.on("TrueFiToken:setup", async ({ context }) => {
  ...
  ponder.on("TrueFiToken:Transfer", async ({ event, context }) => {
  ...
  ponder.on(`TrueFiToken:DelegateChanged`, async ({ event, context }) => {
  ...
  ponder.on(`TrueFiToken:DelegateVotesChanged`, async ({ event, context }) => {
