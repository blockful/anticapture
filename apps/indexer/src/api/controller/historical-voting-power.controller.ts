  ...
  app.openapi(
    ...
    async (context) => {
      try {
        ...
        const response = {
          data: votingPowers.map((votingPower) => ({
            address: votingPower.address,
            votingPower: votingPower.votingPower.toString(), // Convert BigInt to string for JSON serialization
            blockNumber: votingPower.blockNumber,
            tokenAddress: votingPower.tokenAddress,
          })),
          metadata: {
        ...
        console.error("Historical voting power endpoint error:", error);
        ...
        return context.json(
          {
            ...
            message:
              "An unexpected error occurred while fetching historical voting power",
          },
          500,
        );
      }
