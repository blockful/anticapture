...
export function useDelegateDelegationHistory({
  ...
  const queryVariables = useMemo(
    () => ({
      ...
      ...(filterVariables?.minDelta && {
        minDelta: filterVariables.minDelta,
      }),
  ...
  const { data, error, loading, fetchMore } = useVotingPowersQuery({
  ...
  const hasNextPage = useMemo(() => {
    return currentPage * itemsPerPage < (data?.votingPowers?.totalCount || 0);
  }, [currentPage, itemsPerPage, data?.votingPowers?.totalCount]);
        ...
        updateQuery: (
          ...
          return {
            votingPowers: {
              ...
              items: [
                ...(previousResult.votingPowers?.items ?? []),
                ...(fetchMoreResult.votingPowers?.items ?? []),
              ],
