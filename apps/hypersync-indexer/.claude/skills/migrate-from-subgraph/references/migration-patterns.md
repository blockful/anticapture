...
const mints = await context.Mint.getWhere({
  transaction_id: { _eq: transactionId },
});
const burns = await context.Burn.getWhere({
  transaction_id: { _eq: transactionId },
});
```
...
```typescript
const existingMint = await context.Mint.getWhere({
  transaction_id: { _eq: transactionId },
});
...
export const getTokenMetadata = createEffect(
  ...
  async ({ input: tokenAddress, context }) => {
    try {
      ...
      context.log.error(
        `Error fetching token metadata for ${tokenAddress}: ${error}`,
      );
      return {
        name: "Unknown",
        symbol: "Unknown",
...
Contract.EventName.handler(async ({ event, context }) => {
  const tokenMetadata = await context.effect(
    getTokenMetadata,
    event.params.token,
  );
