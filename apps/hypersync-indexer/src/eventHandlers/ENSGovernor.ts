...
indexer.onEvent(
  { contract: "ENSGovernor", event: "VoteCast" },
  async ({ event, context }) => {
...
indexer.onEvent(
  { contract: "ENSGovernor", event: "ProposalCreated" },
  async ({ event, context }) => {
...
indexer.onEvent(
  { contract: "ENSGovernor", event: "ProposalCanceled" },
  async ({ event, context }) => {
...
indexer.onEvent(
  { contract: "ENSGovernor", event: "ProposalExecuted" },
  async ({ event, context }) => {
    await updateProposalStatus(
      context,
      event.params.proposalId.toString(),
...
indexer.onEvent(
  { contract: "ENSGovernor", event: "ProposalQueued" },
  async ({ event, context }) => {
    await updateProposalStatus(
      context,
      event.params.proposalId.toString(),
