...
- The Prometheus `/metrics` endpoint can now be guarded by a shared bearer token via the new optional `METRICS_API_TOKEN` env var (set it on the public deployment; left open when unset for local dev). This is independent of per-tenant auth so scrapes never consume a tenant token or appear in per-tenant usage metrics.
